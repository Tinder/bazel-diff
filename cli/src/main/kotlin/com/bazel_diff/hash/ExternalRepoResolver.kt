...
class ExternalRepoResolver(
  ...
  private val externalRoot: Path by lazy { outputBase.resolve("external") }
  ...
  private fun resolveBzlModPath(repoName: String): Path {
    // Query result line should look something like "<exec root>/external/<canonical repo
    // name>/some/bazel/target: <kind> <label>"
    val queryResultLine =
        runProcessAndCaptureFirstLine(
            bazelPath.toString(),
            "query",
            "@$repoName//...",
            "--keep_going",
