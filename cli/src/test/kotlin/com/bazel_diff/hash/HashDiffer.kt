 ...
 * 
 * Allows the creation of test assertions that don't depend on the underlying hash values,
 * but instead assert properties based on expected changes to hash values when comparing
 * the results of hashing two build graphs.
 * 
 * @property from The map of target hashes to compare from.
 * @property to The map of target hashes to compare to.
 */
class HashDiffer(private val from: Map<String, TargetHash>, private val to: Map<String, TargetHash>) {
