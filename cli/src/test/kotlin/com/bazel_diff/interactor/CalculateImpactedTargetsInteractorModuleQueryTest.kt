...
/**
 * Tests for the module-change query path in [CalculateImpactedTargetsInteractor] that exercise the
 * predicate matching changed modules to canonical external repos in `allTargets`. Lives in its own
 * test class (instead of extending `CalculateImpactedTargetsInteractorTest`) so we can register a
 * mocked [BazelQueryService] in Koin without disturbing the existing global `KoinTestRule`.
 */
class CalculateImpactedTargetsInteractorModuleQueryTest : KoinTest {
  ...
  @Test
  fun doesNotMatchUnrelatedRepoBySubstring() {
    runBlocking {
  ...
  @Test
  fun moduleWithNoMaterialisedReposIsNotQueried() {
    runBlocking {
  ...
  @Test
  fun versionBumpReportedAsAddPlusRemoveDedupesCanonical() {
    runBlocking {
  ...
  @Test
  fun queryFailureOnBzlmodOnlyShapeEmitsAllHashedLabels() {
    runBlocking {
  ...
  @Test
  fun queryFailureOnMixedWorkspacePreservesGranularity() {
    runBlocking {
      ...
      assertThat(outputLines(writer)).containsExactlyInAnyOrder("//app:app", "//lib:util")
    }
  }

  @Test
  fun catchFallbackOnMixedWorkspaceStripsExternalUnderExcludeFlag() {
    runBlocking {
  ...
  @Test
  fun catchFallbackOnBzlmodOnlyKeepsCanonicalsUnderExcludeFlag() {
    runBlocking {
      ...
      assertThat(outputLines(writer))
          .containsExactlyInAnyOrder("@@abseil-cpp+//absl:strings", "@@abseil-cpp+//absl:base")
    }
  }

  @Test
  fun executeWithDistancesRunsModuleQueryPath() {
    runBlocking {
