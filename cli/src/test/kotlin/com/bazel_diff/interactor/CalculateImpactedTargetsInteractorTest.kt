...
class CalculateImpactedTargetsInteractorTest : KoinTest {
  ...
  // ------------------------------------------------------------------------
  // Reproducer for https://github.com/Tinder/bazel-diff/issues/268
  // ------------------------------------------------------------------------
  ...
  // impacted Rule whose only changed dependency is a filtered-out GeneratedFile triggers:
  //
  //   InvalidDependencyEdgesException("<label> was indirectly impacted, but has no impacted dependencies.")
  //
  // This crashes the whole job. The user @agustinmista in the issue narrowed the cause
  // to exactly this: a `--targetType=Rule` filter combined with a Rule that depends on a
  // GeneratedFile in the autogenerated portion of their graph. The asked-for fix is to
  // downgrade this from a fatal exception to a warning (with a conservative distance like
  // 0) so the indirectly impacted Rule still appears in the output.
  //
  // Once that fix lands, remove @Ignore from this test.
  @Test
  @org.junit.Ignore(
      "Reproducer for https://github.com/Tinder/bazel-diff/issues/268 - when a Rule is " +
          "indirectly impacted by a dependency that was filtered out via --targetType=Rule, " +
          "computeAllDistances throws InvalidDependencyEdgesException and crashes the job. " +
          "It should fall back to a warning + conservative distance so the indirectly " +
          "impacted Rule still appears in the impacted-targets output.")
  fun computeAllDistances_targetTypeFilteredDep_shouldNotCrash_reproducerForIssue268() {
    ...
    // Today this throws:
    //   //pkg:rule was indirectly impacted, but has no impacted dependencies.
    // After the fix, the call should succeed and //pkg:rule should still be reported.
    val impacted = interactor.computeAllDistances(startHashes, endHashes, depEdges)
    assertThat(impacted.keys).contains("//pkg:rule")
  }
